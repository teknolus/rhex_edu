...
def _clamp(value, limits):
    ...
    if upper is not None:
        value = np.minimum(value, upper)  # Element-wise minimum
    if lower is not None:
        value = np.maximum(value, lower)  # Element-wise maximum
...
class PID(object):
    ...
    def __init__(
        ...
        self._proportional = np.array([0.0]*6)
        self._integral = np.array([0.0]*6)
        self._derivative = np.array([0.0]*6)
...
class IncrementalPID(PID):
    """A simple Incremental PID controller."""

    def __init__(self, *args, **kwargs):
        self._last_errors = deque([0, 0, 0], maxlen=3)
        super(IncrementalPID, self).__init__(*args, **kwargs)
    ...
    def __call__(self, input_, dt=None):
        ...
        e_k_2, e_k_1, e_k = self._last_errors
        # Compute the proportional term
        self._proportional = self.Kp * (e_k - e_k_1)

        # Compute integral and derivative terms
        self._integral = self.Ki * e_k * dt
        self._integral = _clamp(self._integral, self.output_limits)  # Avoid integral windup

        self._derivative = self.Kd * (e_k - 2 * e_k_1 + e_k_2) / dt
    ...
    def __repr__(self):
        return (
            ...
            'Kp={self.Kp!r}, Ki={self.Ki!r}, Kd={self.Kd!r}, '
            'setpoint={self.setpoint!r}, sample_time={self.sample_time!r}, '
            'output_limits={self.output_limits!r}, last_errors={self._last_errors!r}, auto_mode={self.auto_mode!r}, '
            'proportional_on_measurement={self.proportional_on_measurement!r},'
            'error_map={self.error_map!r}'
            ')'
        ).format(self=self)