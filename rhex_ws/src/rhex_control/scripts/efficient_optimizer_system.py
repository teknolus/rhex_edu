...
class GymSystem(gym.Env):
    ...
    def get_reward(self, obs):
        ...
        tol = (0.2 - sum_abs_e) if sum_abs_e <= 0.01  else 0.0
        torque_min = np.sum(np.abs(self.system.u[self.system.k])) * scale  if sum_abs_e <= 0.01 else 0.0
        reward = -sum_e_squared + tol - torque_min 
